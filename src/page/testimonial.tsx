...
const TestimonialCard = ({testimonial, onClick}) => (<div onClick={() => onClick(testimonial)}
        ...
        <p className="xl:line-clamp-2 xs:line-clamp-6 text-xl xs:text-xs">{testimonial.content}</p>
    </div>);
...
const Modal = ({isOpen, onClose, testimonial}) => {
    ...
    return (<div className=" fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center xs:p-6 z-10">
            <div className="bg-white p-6 rounded-md shadow-md max-w-lg w-full">
                ...
                <p className={'text-xl xs:text-xs'}>{testimonial.content}</p>
            </div>
        </div>);
};
...
const Testimonial = () => {
    ...
    return (<div className="xl:px-44 mb-6 xs:px-10 mt-6">
            <h2 className={'mt-12 text-3xl text-center font-bold text-accent mb-8'}>Testimonials</h2>
            <Swiper
                spaceBetween={20}
                slidesPerView={1}
                navigation
